...
companies = json_serializer.serialize(Request.objects.all(), ensure_ascii=False)
...
def processRequest(request, id):
    print("in procesesRequest")
    print(id)
    req = Request.objects.get(pk=id)
    
    req_email = req.req_email
    tutor_email = req.req_email
    admin_email = User.objects.values_list('email', flat=True).get(username='admin1')
    email_from = settings.EMAIL_HOST_USER
    email_subject = ''
    email_message = ''
    ...
    if data.get("decision") is not None:
        ...
        if req.status == "Approved":
            print("inide approved")
            if req.type == 'Become a Tutor':
                tutor_rec = Tutor.objects.get(userstamp=req.requestor)
                tutor_rec.status = 'Active'
                tutor_rec.save()
                email_subject = "Your request to become a tutor has been approved"
                email_message = "Request ID: " + str(id) + " for " + req.requestor + " has been approved."
            if req.type == 'Find a Tutor':
                email_subject = "Your Find a Tutor Request has been approved"
                email_message = "Request ID: " + str(id) + " for " + req.requestor + " to work with tutor, " + req.tutorname + " has been approved."

        if req.status == "Rejected":
            print("inside rejected")
            if req.type == 'Become a Tutor':
                email_subject = "Your request to become a tutor has been rejected"
                email_message = "Request ID: " + str(id) + " for " + req.requestor + " has been rejected."
            if req.type == 'Find a Tutor':
                email_subject = "Your Find a Tutor Request has been rejected"
                email_message = "Request ID: " + str(id) + " for " + req.requestor + " to work with tutor, " + req.tutorname + " has been rejected."
...
def editrequest(request):
    ack_message = ''
    if request.method == "POST":
        username = 'admin1' #request.POST["username"]
...
#     title="Requests"
        ...
        requests = Request.objects.filter(requestor=request.user).order_by('-createdDate')
#    JsonResponse([requests.serialize() for req in requests], safe=False)
#    return JsonResponse(serializers.serialize('json', requests), safe=False)
#    if y == 10:
#        requests = Request.objects.all().order_by('-createdDate')
#        title="Requests"
#    else:
#        requests = Request.objects.filter(requestor=request.user).order_by('-createdDate')
#        title="My Requests"
    a = serializers.serialize('json', requests)
    b = json.loads(a)
#    print(type(b[0]))
#    print(HttpResponse(b[0]))
#    return HttpResponseRedirect(b[0])
...
def requestsOne(request):
    if request.user == 'admin1':
        requests = Request.objects.all().order_by('-createdDate')
        print(requests)
    else:
        requests = Request.objects.filter(requestor=request.user).order_by('-createdDate')
